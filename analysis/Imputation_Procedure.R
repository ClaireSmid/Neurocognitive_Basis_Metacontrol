...
# For clarity, we pry apart the flanker and stroop measures, to see what is driving the relation
# to metacontrol specifically

Flank <- read.csv('/Users/claire.smid/Dropbox/Thesis/Flank_T0.csv')
names(Flank)[1] <- "ID"
Flank$Session <- 0
Stroop <- read.csv('/Users/claire.smid/Dropbox/Thesis/Animalstroop_correctedSD_t0.csv')
names(Stroop)[1] <- "ID"
Stroop$Session <- 0

names(Flank)

### calculate the Flank composite scores and accuracy and costs etc
# z scores of accuracy, rt and error
Flank$Flank_Congruent_Correct_Avg_RT_z <- scale(Flank$Flank_Congruent_Correct_Avg_RT,scale=T,center=T)
Flank$Flank_Incongruent_Correct_Avg_RT_z <- scale(Flank$Flank_Incongruent_Correct_Avg_RT,scale=T,center=T)

Flank$Flank_Neutral_error_z <- scale(Flank$Flank_Congruent_error,scale=T,center=T)
Flank$Flank_Incongruent_error_z <- scale(Flank$Flank_Incongruent_error,scale=T,center=T)

Flank$Flank_Congruent_Correct_z <- scale((1-Flank$Flank_Neutral_error),scale=T,center=T)
Flank$Flank_Incongruent_Correct_z <- scale((1-Flank$Flank_Incongruent_error),scale=T,center=T)

# add them together
# the costs for each condition - higher values mean more cost for each condition
Flank$Flank_Cong_Cost_t <- Flank$Flank_Neutral_error_z + Flank$Flank_Congruent_Correct_Avg_RT_z
Flank$Flank_Incon_Cost_t <- Flank$Flank_Incongruent_error_z + Flank$Flank_Incongruent_Correct_Avg_RT_z

# as numeric
Flank$Flank_Cong_Cost_t <- as.numeric(Flank$Flank_Cong_Cost_t)
Flank$Flank_Incon_Cost_t <- as.numeric(Flank$Flank_Incon_Cost_t)

# flank extract
names(Flank)
Flank_vars <- c("ID","Session","Flank_Cong_Cost_t","Flank_Incon_Cost_t")

Flank_vs <- Flank[, (names(Flank) %in% Flank_vars)]

DF <- left_join(DF_s,Flank_vs,by=c("ID","Session"))
names(DF)
